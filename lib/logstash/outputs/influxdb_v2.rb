...
# An influxdb_v2 output that does nothing.
class LogStash::Outputs::InfluxdbV2 < LogStash::Outputs::Base
  include Stud::Buffer
  config_name "influxdb_v2"
 
  # The database to write - supports sprintf formatting
  config :bucket, :validate => :string, :default => "statistics"
  ...
  config :org, :validate => :string, :required => true
...
def flush(events, teardown = false)
    @logger.debug? and @logger.debug("Flushing #{events.size} events to  chenzhigang - Teardown? #{teardown}")
    dowrite(events)
  end # def flush
    
  def dowrite(events)
    begin
      @write_api.write(data: events)
      #@write_api.write(data: {name: 'hchen',tags: { host: 'aws', region: 'us' },fields: { level: 5, saturation: '99%' },time:Time.now.utc})
...
end # class LogStash::Outputs::InfluxdbV2
